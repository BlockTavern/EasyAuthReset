...
- 💾 验证码 / 邮箱绑定关系磁盘持久化（**服务器重启不丢**）；冷却仅内存（重启清空，运维重启不惩罚玩家）
- 🧪 启动自动探测 SMTP 可达性；管理命令 `/easyauthreset test <邮箱>` 一键发测试邮件
...
| `/easyauthreset test <邮箱>` | **OP/控制台**：发送测试邮件验证 SMTP 配置（无需等玩家操作） |

### 玩家提示一览（每个操作都有明确反馈）

| 场景 | 玩家看到 |
|---|---|
| 申请成功（未绑定，输入邮箱） | 🟢 正在发送验证码至 {邮箱}，请稍候… → 随后"验证码已发送至 {邮箱}…（若被踢出可直接 confirm）" |
| 申请成功（已绑定/已登记，输入任何邮箱都会被忽略） | 🟢 该账号已绑定邮箱，正在向绑定邮箱 {邮箱} 发送验证码…（后段同上） |
| 邮件发送失败 | 🔴 验证码邮件发送失败，请检查邮箱地址或稍后重试（已进入冷却） |
| 冷却中 | 🔴 操作过于频繁，请等待 {N} 秒后再试 |
| 未注册账号 | 🔴 该账号未在服务器注册，无法重置密码 |
| 邮箱格式无效 | 🔴 邮箱格式无效，请检查后重试 |
| 要求先绑定 | 🟡 该账号尚未绑定邮箱，请先执行 /resetpassword bind <你的邮箱> 验证并绑定 |
| 服主登记账号却试 bind | 🟡 该账号已由服主登记邮箱，无需自助绑定 |
| 验证码错误/过期 | 🔴 验证码错误或已过期，请重新申请 |
| 验证码尝试超限 | 🔴 验证码错误次数过多，已作废，请重新申请 |
| 重置成功 | 🟢 密码已重置！临时密码：{xx} /login 指引 / 改密指引（邮件同步发送，失败另有提示） |
| 绑定成功 | 🟢 绑定成功！该账号的邮箱为 {邮箱}，之后重置密码只会发送到该邮箱。现在可直接输入 /resetpassword |
| 换绑成功提示 | 🟢 绑定成功…（换绑时申请阶段即提示"将更换绑定邮箱…无需联系管理员"） |
| 绑定邮件带激活链接 | 🟢 绑定验证邮件已发送至 {邮箱}（内含一次性激活链接）… |
| IP 校验拦截（strict） | 🔴 安全保护：当前网络环境与账号历史登录 IP 不同，本次操作已暂停并向管理员告警 |
| IP 校验放行（warn） | 🟡 安全提示：…已放行并向管理员告警 |
| 仅管理员可见 | ⚠️ 安全告警（游戏内给 OP）+ 告警邮件（alertEmail）+ 日志 |

> 所有提示均为 `language: zh/en` 双语 + `messagePrefix` 前缀；服务器端日志为英文（管理员向）。
...
- **验证码**：一次性、过期、错误上限（默认 5 次）、用途区分（重置/绑定）；**磁盘持久化**（重启不丢）
- **冷却**：同 UUID 默认 5 分钟一次申请（发送失败也计入冷却）；**仅内存**，服务器重启自动清空（运维重启不惩罚玩家）
