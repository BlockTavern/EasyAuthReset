...
public class EasyAuthResetConfig {
    ...
    private boolean apply(JsonObject obj) {
        ...
        for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
            ...
            if (!value.isJsonPrimitive()) {
                // "_说明" 是生成模板时的说明信息，不是配置项，忽略且不触发重写
                if (!"_说明".equals(entry.getKey())) {
                    dirty = true;
                }
                continue;
            }
    ...
    public void save() {
        ...
        try (Writer writer = Files.newBufferedWriter(path)) {
            ...
            out.add("_说明", buildHints());
            JsonObject fields = gson.toJsonTree(this).getAsJsonObject();
            fields.entrySet().forEach(entry -> out.add(entry.getKey(), entry.getValue()));
    ...
    /** 每字段的中文说明（写进配置文件本身，玩家/服主可直接读懂每项填什么）。 */
    private static JsonObject buildHints() {
        JsonObject h = new JsonObject();
        h.addProperty("smtpHost", "SMTP 服务器地址（Gmail 默认 smtp.gmail.com；其他服务商照抄其 SMTP 地址）");
        h.addProperty("smtpPort", "SMTP 端口（Gmail 587；465 则配合 smtpSsl）");
        ...
        h.addProperty("codeLength", "验证码位数（4-10）");
        h.addProperty("maxCodeAttempts", "验证码允许的错误尝试次数，超限立即作废");
        h.addProperty("tempPasswordLength", "重置后临时密码的长度（8-64）");
        h.addProperty("showTempPasswordInChat", "是否在游戏内私聊中显示临时密码 true=显示（SMTP故障时玩家能自救）；false=只发邮件");
        h.addProperty("cooldownSeconds", "同一玩家两次申请验证码的最短间隔（秒，10-86400）");
        h.addProperty("requireEmailBind", "true=玩家必须绑定/登记过邮箱才能重置（推荐，玩家输入的邮箱不会收到验证码）；false=允许直接输邮箱（有误填风险）");
        h.addProperty("enableClickActivation", "true=绑定验证邮件附带一次性激活链接（点链接即完成绑定）；false=只用6位验证码（无需开放端口）");
        h.addProperty("activationHttpPort", "点击激活链接用的端口（enableClickActivation=true 时需在防火墙/云安全组放行）");
        h.addProperty("activationHttpBind", "监听地址：0.0.0.0=所有网卡（外部可访问）；127.0.0.1=仅本机（配合Nginx反代）");
        h.addProperty("activationPublicUrl", "玩家浏览器访问的公开地址（不带结尾斜杠），如 http://你的域名或公网IP:8123；Nginx 反代 HTTPS 场景填 https://你的域名；留空则邮件里没有链接");
        h.addProperty("language", "玩家消息语言：zh=中文 / en=英文");
        h.addProperty("messagePrefix", "玩家消息前缀（可留空），如 §e[密码重置]§r ");
        h.addProperty("ipCheckMode", "off=不校验；warn=当前IP与账号历史登录IP不同时仅告警（推荐）；strict=不同则拦截操作并告警（可能误伤换IP玩家）");
        h.addProperty("alertEmail", "管理员告警邮箱：安全事件（IP差异等）发到这里的邮箱；留空则只发游戏内给OP+日志");
        h.addProperty("verificationSubject", "验证码邮件主题");
        h.addProperty("newPasswordSubject", "新密码邮件主题");
        return h;
    }
}
